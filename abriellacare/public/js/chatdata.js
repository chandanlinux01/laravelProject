	...
	commentsRef.on('child_added', (data) => {
		...
		if (data.key != $("#mycurrentuser").val()&&$("#search" + data.key).length==0&&str.usertype!=$("#usertype").val()) {
				...
				if (str.profile != "") {
					...
					var path = $("#siteurl").val() + '/public/upload/profile.png';
				}
...
function getreceiverinfo(receiverid) {
	...
	starCountRef.on('value', (snapshot) => {
		...
		if (data.profile == "") {
			$("#receiverimg").attr("src", $("#path_url").val() + "/public/upload/profile.png");
		} else {
		...
		if (ownchatlist != null) {
			...
			starCountRef1.once('value', (snapshot1) => {
				...
				if (user != null) {
					...
					if (user.profile == "") {
						var path = $("#path_url").val() + "/public/upload/profile.png";
